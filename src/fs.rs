...
impl File {
    pub async fn open_with(exec: &Executor, opt: OpenOptions, path: impl AsRef<Path>) -> Result<Self, UringError> {
        struct OpenFut<'a> {
            key: Option<IoKey>,
            path: Option<PathBuf>,
            opt: OpenOptions,
        ...
        impl<'a> Future for OpenFut<'a> {
            type Output = Result<FileHandle, UringError>;
    ...
    pub async fn open(exec: &Executor, path: impl AsRef<Path>) -> Result<Self, UringError> {
        let opt = OpenOptions::new().read(true);
        Self::open_with(exec, opt, path).await
    }

    pub async fn create(exec: &Executor, path: impl AsRef<Path>) -> Result<Self, UringError> {
