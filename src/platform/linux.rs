...
impl Platform {
    pub fn new() -> Result<Self, UringError> {
        ...
        if !ring.params().is_feature_submit_stable() {
            return Err(UringError::UnsupportedFeature("submit_stable"));
        }
    ...
    fn submit_sqe(&mut self, sqe: squeue::Entry) -> Result<(), UringError> {
        loop {
            let res = unsafe {
                self.ring
                    .submission()
            ...
            match res {
                ...
                Err(_) => {
                    self.ring
                        .submit()
                        .map_err(|err| UringError::SubmitFailed(err))?;
    ...
    // Timeouts
    pub fn push_timeout(&mut self, dur: Duration) -> Result<IoKey, UringError> {
    ...
    pub fn cancel_timeout(&mut self, key: IoKey) -> Result<(), UringError> {
    ...
    // File Opens
    pub fn push_file_open(&mut self, opt: &OpenOptions, path: PathBuf) -> Result<IoKey, UringError> {
        ...
        let mut flags = if opt.read && !opt.write {
            ...
            return Err(UringError::IOError(io::Error::from(io::ErrorKind::InvalidInput)));
        };
    ...
    pub fn pop_file_open(&mut self, key: IoKey) -> Option<Result<FileHandle, UringError>> {
        self.io_map
            .remove(key)
            .map(|entry| match entry {
                IoMapEntry::FileOpen(res) => res.map(|res| res.map_err(|err| UringError::IOError(err))),
                _ => panic!("IoMapEntry in unexpected state")
            })
            .expect("Couldn't find IoMapEntry for this key")
    }
    ...
    pub fn wait_for_events(&mut self) -> Result<(), UringError> {
        self.ring
            .submit_and_wait(1)
            .map_err(|err| UringError::SubmitFailed(err))?;
