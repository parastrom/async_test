...
impl Executor {
    ...
    // Timeouts
    pub fn push_timeout(&self, dur: Duration) -> Result<IoKey, UringError> {
        self.plat.borrow_mut().push_timeout(dur)
    }

    pub fn cancel_timeout(&self, key: IoKey) -> Result<(), UringError> {
    ...
    // File Opens
    pub fn push_file_open(&self, opt: &OpenOptions, path: PathBuf) -> Result<IoKey, UringError> {
        self.plat.borrow_mut().push_file_open(opt, path)
    }

    pub fn pop_file_open(&self, key: IoKey) -> Option<Result<FileHandle, UringError>> {
        self.plat.borrow_mut().pop_file_open(key)
    }


    pub fn wait_for_events(&self) -> Result<(), UringError> {
